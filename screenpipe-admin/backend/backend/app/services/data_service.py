        ...
        for frame in report.data.frames:
            if hasattr(frame, 'ocr_text') and frame.ocr_text:
                ocr_doc = {
                    ...
                    "window_name": frame.window_name if hasattr(frame, 'window_name') and frame.window_name is not None else "",
                    "focused": frame.focused if hasattr(frame, 'focused') else False,
                    "text_length": frame.ocr_text.text_length if hasattr(frame.ocr_text, 'text_length') and frame.ocr_text.text_length is not None else len(frame.ocr_text.text),
