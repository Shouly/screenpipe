...
async def email_login(
    ...
    try:
        ...
        # 发送邮件（异步）
        background_tasks.add_task(EmailService.send_login_code, login_data.email, code)
...
async def verify_email_login(
    request: Request, verify_data: dict, db: AsyncSession = Depends(get_db)
):
    """
    验证邮箱登录码
    """
    try:
        ...
        if not is_valid:
            ...
            raise HTTPException(
                status_code=status.HTTP_401_UNAUTHORIZED, detail="验证码无效或已过期"
            )
            ...
            if user_agent:
                ...
                if "Windows" in user_agent:
                    ...
                    mac_version = re.search(
                        r"Mac OS X (\d+[._]\d+[._]?\d*)", user_agent
                    )
                    if mac_version:
                    ...
                    android_version = re.search(
                        r"Android (\d+[._]\d+[._]?\d*)", user_agent
                    )
                    if android_version:
        ...
        user, is_new_user = await UserService.login_or_register_by_email(
            db, email, device_info=device_info, ip_address=client_ip
        )
...
async def oauth_login(
    request: Request, login_data: OAuthLoginRequest, db: AsyncSession = Depends(get_db)
):
    """
    OAuth登录
    """
    try:
        ...
        # 验证OAuth令牌
        user_info = await UserService.verify_oauth_token(
            login_data.provider, login_data.token
        )

        if not user_info:
            logger.warning(
                f"OAuth令牌验证失败: provider={login_data.provider}, ip={client_ip}"
            )
            raise HTTPException(
                status_code=status.HTTP_401_UNAUTHORIZED, detail="无效的OAuth令牌"
            )
        ...
        if not oauth_id or not email:
            logger.warning(
                f"OAuth令牌缺少必要信息: provider={login_data.provider}, ip={client_ip}"
            )
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
            ...
            try:
                ...
                if user_agent:
                    ...
                    if "Chrome" in user_agent and "Chromium" not in user_agent:
                        ...
                        safari_version = re.search(
                            r"Version/(\d+\.\d+\.\d+)", user_agent
                        )
                        if safari_version:
                            browser_version = safari_version.group(1)
                    elif "Edge" in user_agent:
                        ...
                        edge_version = re.search(
                            r"Edge/(\d+\.\d+\.\d+\.\d+)", user_agent
                        )
                        if edge_version:
                            browser_version = edge_version.group(1)
...
async def get_current_user(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED, detail=f"无效的认证令牌: {str(e)}"
        )
